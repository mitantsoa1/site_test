...
<body>
    ...
    <div class="adresse_content">ici</div>  
    <?php
        ...
        $_SESSION['matiere']='Malagasy';
            ...
            include_once('Models/Database.class.php');
                    ...
                    for($i=0;$i<count($t);$i++){
                        // 
                        if($t[$i]->nom ==ucwords($_SESSION['user']) && $t[$i]->pass == sha1($_SESSION['pass']) && $t[$i]->mail == $_SESSION['mail'] ){
                            
                            $table=$db ->select('user')
                                        ->where('nom','=')
                                        ->et('pass','=')
                                        ->et('mail',"=")
                                        ->execute([ucwords($_SESSION['user']),sha1($_SESSION['pass']),$_SESSION['mail']]);
                            $id_prof = $table[0]->id;
                            ...
                            for($i=0;$i<count($tab);$i++){
                               if(count($tab)>0 && ($tab[$i]->nom == $_SESSION['matiere'])){
                                   $temp = true;
                                     break; 
                               }      
                            }
                            if($temp == true){
                                echo '<div><button id="publier_modal" data-toggle="modal" data-target="#Modal" class="btn btn-primary publier_modal sticky-top">Publier</button></div> ';
                            }
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form id="form_malagasy" enctype="multipart/form-data">
                            <?php 
                                Controllers::includeTemplate("form_publication.php");
                            ?>
                            <button id="btn_publier_malagasy" class="btn btn-primary text-center col-sm-offset-2 col-sm-2 ">Publier</button>
                        </form>
                    </div>
