...
function selectionAll() {
    ...
    s.sendInfo("Publier/selectAllPub", function(data) {
        ...
        var table_affichage = "<div class= "
        '+$table[$i].categorie+'
        "> <table><thead style='position: sticky !important;z-index: 1000;'><th>ID</th><th>Nom</th><th>Prenom</th><th>Actions</th></thead><tbody>";
        for (var i = 0; i < data.length; i++) {
            table_affichage = table_affichage + '<tr><td>' + data[i].id + '</td><td>' + data[i].nom + '</td><td>' + data[i].prenom + '</td><td><a href="#" class="effacer" data-id="' + data[i].id + '" value="' + data[i].id + '"><i class="fas fa-trash"></i></a><a href="#" class="modifier"  data-toggle="tooltip"  data-placement="bottom" title="modifier" data-id="' + data[i].id + '" value="' + data[i].id + '" ><i class="fas fa-edit"></i></a>';
        }
